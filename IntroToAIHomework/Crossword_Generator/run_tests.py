...
words_count = [0] * 15
words_avg_fitness = [0] * 15
words_max_fitness = [-100000] * 15

for i in range(1, 101):
    with open(f'inputs/input{i}.txt', 'r') as f:
        words = f.read().splitlines()
    count_of_words = len(words)
    words_count[count_of_words] += 1
    ...
    best_info = parents[0]
    maxfit = evaluate_fitness(parents[0])
    words_max_fitness[count_of_words] = max(words_max_fitness[count_of_words], maxfit)
    words_avg_fitness[count_of_words] += sum([evaluate_fitness(info) for info in parents]) / POP_SIZE
    with open(f"outputs/output{i}.txt", "w") as f:
        for word in words:
            x, y, t = best_info[word]
            t = (t + 1) % 2
            f.write(f'{x} {y} {t}\n')
    print(f"Test {i} is done")

for i in range(15):
    if words_count[i]:
        words_avg_fitness[i] /= words_count[i]

with open("statistics.txt", "w") as f:
    f.write(f'{words_max_fitness}')
    f.write(f'{words_avg_fitness}')
print(words_max_fitness)
print(words_avg_fitness)
